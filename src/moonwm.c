...
clientmessage(XEvent *e)
{
	...
	if (showsystray && cme->window == systray->win && cme->message_type == atoms[NetSystemTrayOP]) {
		...
		if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
			...
			updatesystrayicongeom(dpy, c, wa.width, wa.height);
...
propertynotify(XEvent *e)
{
	...
	if ((c = wintosystrayicon(ev->window))) {
		if (ev->atom == XA_WM_NORMAL_HINTS) {
			updatesizehints(dpy, c);
			updatesystrayicongeom(dpy, c, c->w, c->h);
		}
		else
			updatesystrayiconstate(dpy, c, ev);
		resizebarwin(selmon);
		updatesystray();
	}
